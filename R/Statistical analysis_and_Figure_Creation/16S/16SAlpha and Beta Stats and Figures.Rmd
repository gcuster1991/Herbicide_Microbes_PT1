...
mod_abund<-function(counts, DV = "Independent Variable", Grouping = "Grouping Variable") {
  data <- counts[counts[,Grouping[i]] == as.character(unique(counts[,Grouping]))[j], ]
  ...
  data<-counts[counts[,Grouping[i]] == as.character(unique(counts[,Grouping]))[j], ]
   #return(boxplot(unlist(data[1]) ~ matrix(data[,DV]), main =  paste(names(data[1])), xlab= "Time", ylab="Abundance")
    #      )
   return(boxplot(unlist(data[1]) ~ matrix(data[,DV]), main =    paste(names(data[1]), as.character(unique(counts[,Grouping]))[j]), xlab= "Time", ylab="Abundance") )
...
#now I need to make this into a for loop for the funciton mod_abund. 
for (i in 1:nreturns){
plot  <- mod_abund(counts = test[,i], DV = test[,"Time"], Grouping = test[,"Herbicide"] )
}
