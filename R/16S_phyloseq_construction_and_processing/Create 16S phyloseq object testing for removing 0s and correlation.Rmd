...
for(i in 1:length(samp_names)){
  for(j in 1:length(samp_names)){
    if(samp_names[i] == samp_names[j]){
      #if(rowSums(cbind(count_tab2[i], count_tab2[j])) < 1){
        tmp2<-cbind(count_tab2[i], count_tab2[j])
        tmp2<-tmp2[rowSums(tmp2)>2, ]
       if(cor(count_tab2[i], count_tab2[j]) < 0.999){
          tmp<-cor(tmp2[1], tmp2[2])
...
#cor_df_with_zeros<-cor_df
cor_df_with_zeros <- data.frame(SampleID = rep(names(cor_df_with_zeros), sapply(cor_df_with_zeros, length)),
                 Correlation = unlist(cor_df_with_zeros))
density(cor_df_with_zeros$Correlation)
