...
```{r}
load("../data/Herb_PS_Objects_ITS.RData")
```

Read in HMC proportions to update Time phyloseq objects 
```{r}
HMC_T1<-read.csv("../data/ITS_time1_otu.csv_p_estimatesHMC.csv")
HMC_T2<-read.csv("../data/ITS_time2_otu.csv_p_estimatesHMC.csv")
HMC_T3<-read.csv("../data/ITS_time3_otu.csv_p_estimatesHMC.csv")
```
...
betadispr_adonis<-function(x){
  x_dist <- phyloseq::distance(x, method = "bray")
    sampledf <- data.frame(sample_data(x))
      beta_mod<-vegan::betadisper(x_dist, sampledf$Herbicide)
