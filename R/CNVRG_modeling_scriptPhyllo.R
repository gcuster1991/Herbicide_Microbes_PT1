...
#For troubleshooting
# dat <- read.csv("./forModeling_16S_otuTables/phyllo_16S_time1_otu.csv",
#                 fill = T, header = T, stringsAsFactors = F)

#Order by treatment
dat <- dat[order(dat$Herbicide),]
dat <- data.frame(dat$Sample_ID, dat$Herbicide, dat[,52:(length(dat))])
...
treatments <- dat2$dat.Herbicide

#remove treatment vector from dataframe
dat2 <- dat2[,-2]

#remove super rare stuff
...
write.csv(forExport, file = paste(inargs[1], "phyllo_p_estimates.csv", sep = ""))
save.image(file = paste(inargs[1], "phyllo.Rdata", sep = ""))
